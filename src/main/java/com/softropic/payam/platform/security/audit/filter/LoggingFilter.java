package com.softropic.payam.platform.security.audit.filter;
...
public class LoggingFilter extends OncePerRequestFilter {
    ...
    private static String deriveOperation(String method, String path) {
        if (path.startsWith("/v1/payments") && "POST".equalsIgnoreCase(method)) {
            return "initiate_payment";
        }
        if (path.startsWith("/v1/callbacks/orange")) {
            return "orange_callback";
        }
        if (path.startsWith("/v1/callbacks/mtn")) {
            return "mtn_callback";
        }
        if (path.startsWith("/v1/admin/transactions")) {
            return "admin_transactions";
        }
        if (path.startsWith("/v1/admin/metrics")) {
