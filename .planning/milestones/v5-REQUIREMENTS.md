# Requirements Archive: v5 Tenant & API Key Management Service Layer

**Archived:** 2026-04-06
**Status:** SHIPPED

**Scope of v5:** Phases 27–29 (service layer). Requirements requiring HTTP endpoints or Admin UI deferred to v6 (phases 30+).
...
- [~] **TENT-02**: Admin can update a tenant's name *(service layer done — Phase 28; HTTP endpoint deferred to v6)*
- [~] **TENT-03**: Admin can update a tenant's email address (optional field used for notifications) *(service layer done — Phase 28; HTTP endpoint deferred to v6)*
- [~] **TENT-04**: Admin can update a tenant's webhookUrl *(service layer done — Phase 28; HTTP endpoint deferred to v6)*
- [ ] **TENT-05**: Admin can view a paginated, searchable list of all tenants with their status *(deferred to v6)*
- [ ] **TENT-06**: Admin can view a tenant's detail page (name, email, TenantRef, status, webhookUrl) *(deferred to v6)*
- [~] **TENT-07**: Admin can suspend an active tenant; all API keys across all environments are immediately revoked *(service layer done with atomic bulk JPQL — Phase 28; HTTP endpoint deferred to v6)*
- [~] **TENT-08**: Admin can reactivate a suspended tenant; system automatically generates a new PROD key shown to admin exactly once *(service layer done — Phase 28; HTTP endpoint deferred to v6)*
- [ ] **TENT-09**: Tenant with SUSPENDED status cannot process API requests (auth rejects all keys regardless of key status) *(deferred to v6)*
...
- [x] **AKEY-05**: System automatically moves ROTATED keys to REVOKED status after 24 hours via an automated job *(Quartz cleanup job — Phase 29)*
...
- [~] **WSEC-03**: Admin can trigger regeneration of the WebhookSecret; new secret replaces the old one *(service layer done — Phase 28; HTTP endpoint deferred to v6)*
