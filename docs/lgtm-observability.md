# Payam — LGTM Stack Observability Guide

**Stack:** Loki · Grafana · Tempo · Mimir/Prometheus  
**Application:** Payam — Mobile Money Payment Orchestration (Orange Money + MTN MoMo)
...
```logql
{service="payam", level="error"} |= "initiate_payment"
```
...
```logql
{service="payam", level="error"} |= "FAILED_PERMANENT"
```
...
```logql
{service="payam", level="error"} |= "OrangeTokenService"
```

**MTN token refresh failures**
```logql
{service="payam", level="error"} |= "MtnTokenService"
```
...
```logql
{service="payam"} |= "ReconciliationJob"
```

**Reconciliation discrepancies found**
```logql
{service="payam", level="error"} |= "ReconciliationService"
  | json
...
```logql
{service="payam"} |= "CallNotPermittedException"
```
...
```logql
{service="payam"} |= "PAYMENT_ALREADY_PROCESSING"
  | json
...
```logql
{service="payam", level="error"} |= "IdempotencyService"
```
...
```logql
sum(rate({service="payam", level="error"} |= "initiate_payment" [1m]))
```
...
Payam exposes metrics via Micrometer at `/manage/prometheus`. All metrics carry the tag `application="payam"`.
...
**Live thread count**
```promql
jvm_threads_live_threads{application="payam"}
```

**Database connection pool active / max**
```promql
hikaricp_connections_active{application="payam"}
hikaricp_connections_max{application="payam"}
```
...
```logql
{service="payam"} |= "status_poller" |= "PROCESSING"
```
...
sum by (errorCode) (
  count_over_time(
    {service="payam", level="error"} | json | __error__="" [5m]
  )
)
```
...
```logql
count_over_time(
  {service="payam", level="error"} |= "webhook_delivery" [5m]
)
```
...
### 6.1 Payam — Operations Overview (Golden Signals)
...
| Recent Error Logs | LogQL: `{service="payam", level="error"}` | Logs panel |
...
| Panel | Query | Visualization |
|-------|-------|---------------|
| Reconciliation Job Last Run | `{service="payam"} \|= "ReconciliationJob" \|= "completed"` | Logs panel |
| HIGH Severity Discrepancies | `{service="payam", level="error"} \| json \| severity="HIGH"` | Table |
| Discrepancy Types | LogQL count grouped by `discrepancyType` | Bar chart |
| Webhook Delivery Failures | `count_over_time({service="payam", level="error"} \|= "webhook_delivery" [5m])` | Stat |
| Permanently Failed Webhooks | `count_over_time({service="payam", level="error"} \|= "FAILED_PERMANENT" [24h])` | Stat |
| Idempotency Key Collisions | `count_over_time({service="payam"} \|= "PAYMENT_ALREADY_PROCESSING" [5m])` | Time series |
...
3. **Drill to Loki** → filter `{service="payam", level="error"}`, narrow time range
