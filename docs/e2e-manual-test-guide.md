...
3.  **Step:** Manually simulate a successful callback from MTN (PUT request to `/api/webhooks/mtn`).
...
3.  **Step:** Simulate Orange callback (POST to `/api/webhooks/orange`).
4.  **Verify:** Transaction status transitions to `SUCCESS`.

---

## 3. Transaction Integrity & Invariants
...
### 3.3 Idempotency & Replay Protection
...
### 4.2 Illegal Transitions (Invariant Preservation)
...
### 5.1 Webhook Failure & Polling Fallback
...
## 6. Event & Integration Correctness

### 6.1 Webhook Double-Check Pattern
1.  **Step:** Send a fake successful webhook to `/api/webhooks/mtn` for a real pending transaction.
2.  **Verify Logs:** Ensure the `WebhookDoubleCheckHandler` fires a `GET` request to MTN's status API *before* updating the DB.
3.  **Verify:** If the provider API says "FAILED", the transaction must move to `FAILED` despite the webhook saying "SUCCESS".

### 6.2 Outbound Webhook Delivery
...
## 7. Fraud Engine Verification

### 7.1 Velocity Blocking
1.  **Step:** Send 11 payment requests from the same IP within 60 seconds (Threshold is 10).
2.  **Verify:** The 11th request returns `HTTP 422` with code `FRAUD_BLOCKED`.
3.  **Verify:** No request was sent to the provider for the 11th attempt.

### 7.2 Risk Score Persistence
...
## 8. Mutation Testing Strategy (Manual)
...
## 9. Final System State Consistency
